...
def job():
    return J.Job('Job 1', C.Run(157.125, 27.875, 24),
                 desc='Typical dimensions, no fillers (default), no legs.')


def test_job_header(job):
    assert job.header == [
        'Job Name: Job 1',
        'Description: Typical dimensions, no fillers (default), no legs.',
        'Total Wall Space: 157.125"']


def test_job_summaryln(job):
    assert job.summaryln == [
        '5 cabinets measuring 30 13/16" totalling 154 1/8"'
...
def test_job_partslist(job):
    assert job.partslist == [
        'Back Panels:      5  @  30 13/16"   x  27 7/8"     x  3/4"',
        'Bottom Panels:    5  @  29 3/8-"    x  22 3/8"     x  3/4"',
        'Side Panels:     10  @  22 3/8"     x  27 7/8"     x  3/4"',
        'Top Nailers:     10  @  29 3/8-"    x   4"         x  3/4"',
        'Doors:           10  @  15 1/4-"    x  27 3/8"     x  3/4"']
...
def test_job_filler_l_summaryln(job_filler_l):
    assert job_filler_l.summaryln == [
        '5 cabinets measuring 36" totalling 180"'
